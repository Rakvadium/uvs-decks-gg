...
export function DeckGridItem({ deck, showAuthor = false }: DeckGridItemProps) {
  ...
  return (
    <Link href={`/decks/${deck._id}`}>
      <div className="group relative flex h-[140px] rounded-lg overflow-hidden border border-border/50 bg-card/50 backdrop-blur-sm hover:border-primary/50 hover:shadow-[0_0_30px_-10px_var(--primary)] transition-all duration-300">
        ...
        <div className="relative w-[100px] shrink-0 bg-gradient-to-br from-primary/10 via-card to-secondary/10 overflow-hidden">
          {displayImage ? (
            <>
              <Image
                src={displayImage}
          ...
          <div className="absolute bottom-1 left-1">
            {deck.isPublic ? (
              <div className="p-1 rounded bg-primary/30 backdrop-blur-sm border border-primary/40">
                <Globe className="h-2.5 w-2.5 text-primary" />
              </div>
            ) : (
              <div className="p-1 rounded bg-muted/60 backdrop-blur-sm border border-border/50">
        ...
        <div className="flex-1 flex flex-col p-3 min-w-0 relative">
          ...
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className="font-display font-bold text-sm uppercase tracking-wide group-hover:text-primary transition-colors truncate">
              {deck.name}
            </h3>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground/30 group-hover:text-primary group-hover:translate-x-0.5 transition-all" />
          </div>
          ...
          {deck.description ? (
            <p className="text-[11px] text-muted-foreground line-clamp-2 mb-auto leading-relaxed">
              {deck.description}
            </p>
          ) : startingCharacter ? (
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-border/30">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1" title="Main Deck">
                ...
                <span className="text-xs font-mono font-bold text-foreground">{mainCount}</span>
              </div>
              
              {sideCount > 0 && (
                <div className="flex items-center gap-1 text-muted-foreground" title="Sideboard">
                  <BookOpen className="h-3 w-3" />
                  <span className="text-[10px] font-mono">{sideCount}</span>
                </div>
              )}
              ...
              {referenceCount > 0 && (
                <div className="flex items-center gap-1 text-muted-foreground" title="Reference">
                  ...
                  <span className="text-[10px] font-mono">{referenceCount}</span>
                </div>
              )}
            </div>
            ...
            <div className={cn(
              "ml-auto flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-mono uppercase tracking-wider",
              mainCount >= 60 
